  ...
  function create(config={}){
    ...
    function materialUpgradeReasons(prev,next,goal,assessment){
      ...
      if(goal==='arc'){
        ...
        const strategy=assessment?.name?mareStrategy(assessment.name):null;
        const improveSp=!!strategy?.improve?.includes('SP')||spNeedsSupport;
        const improveSt=!!strategy?.improve?.includes('ST')||stNeedsSupport;
        const speedCrossRelevant=!!assessment?.abilityKnown&&(!highMother||improveSp);
        const longCrossRelevant=!!assessment?.abilityKnown&&(!highMother||improveSt);
        const speedCrossKeepsTarget=highMother?b.sp>=a.sp:b.sp>=a.sp-1;
        const longCrossKeepsTarget=highMother?b.st>=a.st:b.st>=a.st-1;
        if(speedCrossRelevant&&!a.speedCross&&b.speedCross&&speedCrossKeepsTarget&&b.st>=a.st-1)reasons.push('最終配合で速力/短距離クロスが新たに成立し、補強対象SPとST水準を維持');
        if(longCrossRelevant&&!a.longDistanceCross&&b.longDistanceCross&&b.sp>=a.sp-1&&longCrossKeepsTarget)reasons.push('最終配合で長距離クロスが新たに成立し、補強対象STとSP水準を維持しながら直接のスタミナ補強経路を確保');
        ...
        if(ta&&highMother){
          if(improveSp&&b.sp>=a.sp+2&&b.st>=a.st-1&&tb)reasons.push(`高能力母のST側を守りながらSPを${a.sp}→${b.sp}へ上積み`);
          if(improveSt&&b.st>=a.st+2&&b.sp>=a.sp-1&&tb)reasons.push(`高能力母のSP側を守りながらSTを${a.st}→${b.st}へ上積み`);
          if(!improveSp&&!improveSt&&b.spst>=a.spst+4&&b.sp>=a.sp-1&&b.st>=a.st-1&&tb)reasons.push(`高能力母のSP/STを維持しながら合計を${a.spst}→${b.spst}へ大きく上積み`);
        }else if(b.spst>=a.spst+3&&b.st>=a.st-1){
          reasons.push(`SP+STを${a.spst}→${b.spst}へ改善し、ST低下を抑制`);
        }
        const magnificentRelevant=(speedCrossRelevant&&speedCrossKeepsTarget&&b.speedCross)||(longCrossRelevant&&longCrossKeepsTarget&&b.longDistanceCross);
