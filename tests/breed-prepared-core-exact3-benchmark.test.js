...
function prep(h){
 ...
 const x={nameCanon:core.canon(h.name),ancCanon:a.map(core.canon),ancKey:a.map(core.key)};
 prepCache.set(h,x);return x;
}
function rawCrosses(sire,mare){
 if(!validAnc(sire?.ancestor)||!validAnc(mare?.ancestor))return[];
 const S0=prep(sire),M0=prep(mare),out=[];
 for(let i=0;i<15;i++){
  if(S0.nameCanon&&S0.nameCanon===M0.ancCanon[i])out.push({name:mare.ancestor[i],sireGen:1,mareGen:core.depth(i),sireIndex:-1,mareIndex:i,directSire:true});
  for(let j=0;j<15;j++)if(S0.ancCanon[j]&&S0.ancCanon[j]===M0.ancCanon[i])out.push({name:sire.ancestor[j],sireGen:core.depth(j),mareGen:core.depth(i),sireIndex:j,mareIndex:i,directSire:false});
 }
 return out;
}
function fastDanger(sire,mare){
 ...
 const S0=prep(sire),M0=prep(mare),blocked=new Set(),effective=[];let cnt=0,tyokiken=false,stop2=false,direct=null,twoByTwo=null;
 for(let i=0;i<15;i++){
  if(S0.nameCanon&&S0.nameCanon===M0.ancCanon[i]){
   tyokiken=true;stop2=true;
   if(!direct)direct={name:ma[i],sireGen:1,mareGen:core.depth(i),sireIndex:-1,mareIndex:i,directSire:true};
  }
  if(stop2)continue;
  for(let j=0;j<15;j++){
   if(blocked.has(j+','+i))continue;
   if(S0.ancCanon[j]&&S0.ancCanon[j]===M0.ancCanon[i]){
    cnt++;const x={name:sa[j],sireGen:core.depth(j),mareGen:core.depth(i),sireIndex:j,mareIndex:i,directSire:false};effective.push(x);
 ...
 return{available:true,inbreedCount:cnt,kiken,tyokiken,dangerous:kiken||tyokiken,effectiveCrosses:effective,rawCrosses:rawCrosses(sire,mare),directSireCross:direct,twoByTwo,reason:reasons.join(' / ')};
}
...
// Exact result equivalence over representative direct pairs.
for(const mareName of ['エイスト','スプリングスイーツ','ワカヒルメ']){
 const m=T.broodmares.find(x=>x.name===mareName);
 for(const s of T.stallions){
  const a=base.evaluate(s,m),b=fastEvaluate(s,m);
  if(JSON.stringify(a)!==JSON.stringify(b))throw Error('pair mismatch '+s.name+' x '+mareName);
 }
}
function scan(planner){
 ...
 return{runtimeMs:Date.now()-started,two:two.length,three:n3,r3,b,profiles:Object.fromEntries(['sp','speedCross','production','st','balance'].map(p=>[p,planner.routeKey(r3.profiles?.[p]?.[0])])),bridgeKeys:b.bases.map(planner.routeKey)};
}
prepBuilds=0;prepHits=0;
const A=scan(basePlanner),baseRuntime=A.runtimeMs;
prepBuilds=0;prepHits=0;
const B=scan(fastPlanner),fastRuntime=B.runtimeMs,cacheStats={prepBuilds,prepHits};
...
console.log(JSON.stringify({
 ...
 counts:{two:A.two,three:A.three,bridgeBases:A.bridgeKeys.length},
 timingMs:{canonical:baseRuntime,preparedCache:fastRuntime},
 speedup:baseRuntime/Math.max(1,fastRuntime),
 cacheStats,
 conclusion:'Per-horse normalized ancestor caching preserves exact pair results, profile ranking, and bridge selection through exact third generation.'
},null,2));
