...
need(v27,"<div class=\"mare-use-title\">評価</div>",'mare purpose panel must stay concise and structured');
...
need(v27,'function orderedGoalKeys','purpose rows must sort by recommendation strength');
need(v27,'function primaryGoalKeys','mare summary and attention must share the same recommended-purpose source');
need(v27,'function quickGoalRow','purpose evaluation must use compact rows rather than the old 2x2 copy-heavy grid');
need(v27,'function mareAttentionGroups','attention facts must be grouped instead of flattened');
need(v27,"attentionGroupHtml('加算'",'attention must render additions first');
need(v27,"attentionGroupHtml('減算'",'attention must render subtractions second');
need(v27,"attentionGroupHtml('注意'",'attention must render cautions third');
if(!(v27.indexOf("attentionGroupHtml('加算'")<v27.indexOf("attentionGroupHtml('減算'")&&v27.indexOf("attentionGroupHtml('減算'")<v27.indexOf("attentionGroupHtml('注意'")))throw Error('attention display order must remain 加算→減算→注意');
need(v27,"const attentionGoal=topGoals.includes(goal)?goal:(topGoals[0]||goal)",'attention must follow the recommended purpose unless the selected purpose is itself top-rated');
...
need(v27,'mare-goal-list','purpose evaluations must use one compact ordered list');
need(v27,'mare-attention-group','attention must expose grouped compact rows');
