...
const mares=['スプリングスイーツ','フィットレオタード','ミニミニデート','ワカヒルメ','アマリン'];
const MARE_INDEX=+process.env.MARE_INDEX||0;
if(MARE_INDEX<0||MARE_INDEX>=mares.length)throw Error('invalid MARE_INDEX '+MARE_INDEX);
const mare=mares[MARE_INDEX];
...
function classifyTransition(profile,a,b){
 ...
 const cmp=planner.compareProfile(profile)(b,a),reasons=categoryReasons(profile,a,b);
...
function state(latest){return latest===1?'early-complete':latest===2?'improves-to-2':latest===3?'improves-to-3-conditional':'improves-to-4-conditional'}

const directCollector=planner.createCollector({topN:3,poolN:20});
for(const r of planner.iterateDirect(mare))directCollector.push(r);
const directResult=directCollector.finish();
const firstRoute=directResult.profiles.production?.[0]||directResult.profiles.balance?.[0]||directResult.profiles.sp?.[0];
if(!firstRoute)throw Error('no direct first route');
const first=firstRoute.sires[0];
...
for(let g=2;g<=4;g++){
 ...
 const reasons=advisor.portfolioUpgradeReasons(sireChosen,next),material=reasons.length>0;
 sireTransitions.push({to:g,kind:material?'material':'none',reasons});
 if(material){sireLatest=g;sireChosen=next}
}
categories.sire={label:planner.profileLabels.sire,latestMaterialGeneration:sireLatest,state:state(sireLatest),transitions:sireTransitions,selected:sireChosen};
