...
const lockIndexMap=Array.from({length:15},(_,i)=>Array.from({length:15},(_,j)=>core.descendantLocks(i,j).map(s=>{const [father,mother]=s.split(',').map(Number);return mother*15+father;})));
...
function fastDanger(sire,mare){
 ...
 const S0=prep(sire),M0=prep(mare),blocked=new Uint8Array(225),effective=[],raw=[];let cnt=0,tyokiken=false,stop2=false,direct=null,twoByTwo=null;
 for(let i=0;i<15;i++){
  ...
  for(let j=0;j<15;j++){
   ...
   if(!allowEffective||blocked[i*15+j])continue;
   cnt++;effective.push(x);
   if(i===0&&j===0){tyokiken=true;if(!twoByTwo)twoByTwo=x}
   for(const idx of lockIndexMap[i][j])blocked[idx]=1;
  }
 }
...
console.log(JSON.stringify({
 ...
 conclusion:'Prepared pedigree cache plus single-pass cross scan plus fixed blocked-index mask preserves exact end-to-end results.'
},null,2));
