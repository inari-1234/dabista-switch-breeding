...
if(CASE_INDEX<0||CASE_INDEX>=cases.length)throw Error('invalid CASE_INDEX '+CASE_INDEX);
...
function routeFrom(sires,pair,method,stageCrossItems,stageEffectItems){
 const cp=compactCrossPath(stageCrossItems),ep=compactEffectPath(stageEffectItems);
 return{id:sires.map(x=>key(x)).join('__'),sires:[...sires],generation:sires.length,method,final:compactFinal(pair,sire(sires[sires.length-1])||{name:sires[sires.length-1]}),speedCrossPath:cp.path,materialSpeedCross:cp.material,crossEffectPath:ep.path,materialLongCross:ep.materialLong,finalChild:pair.child};
}
...
function canonical(){
 const t=Date.now(),direct=[...planner.iterateDirect(mare)].find(r=>r.sires[0]===first);
 if(!direct)throw Error('canonical direct missing '+mare+' '+first);
 const c2=planner.createCollector({topN:1,poolN:4});
 ...
 const r4=c4.finish();
 return{runtimeMs:Date.now()-t,direct,r2,r3,r4,future:bestFuture(direct,r2,r3,r4)};
}
function optimized(){
 const t=Date.now(),m=T.broodmares.find(x=>x.name===mare),s1=sire(first),p1=engine.evaluate(s1,m);
 if(!safe(p1))throw Error('unsafe fixed first '+mare+' '+first);
 ...
 for(const r of planner.iterateFourthPreview(mare,b4))c4.push(r);
 const r4=c4.finish();
 return{runtimeMs:Date.now()-t,direct,r2,r3,r4,future:bestFuture(direct,r2,r3,r4),counts:{two:r2.count,three:r3.count,four:r4.count}};
}
const A=canonical(),B=optimized(),compare={},fit={};
for(const p of profiles){
 compare[p]={sameGeneration:A.future[p].generation===B.future[p].generation,sameRoute:JSON.stringify(A.future[p].sires)===JSON.stringify(B.future[p].sires)};
 const selected=B.future[p],route=[B.direct,B.r2.profiles?.[p]?.[0],B.r3.profiles?.[p]?.[0],B.r4.profiles?.[p]?.[0]][selected.generation-1];
 fit[p]={arc:goalFit(route,'arc'),bc:goalFit(route,'bc'),rebuild:goalFit(route,'rebuild'),generation:selected.generation,sires:selected.sires,sp:selected.facts.sp,st:selected.facts.st};
}
if(Object.values(compare).some(x=>!x.sameGeneration||!x.sameRoute))throw Error('optimized mismatch '+mare+' '+first);
const output={passed:true,mare,first,canonicalRuntimeMs:A.runtimeMs,optimizedRuntimeMs:B.runtimeMs,speedup:A.runtimeMs/Math.max(1,B.runtimeMs),optimizedCounts:B.counts,compare,fit};
