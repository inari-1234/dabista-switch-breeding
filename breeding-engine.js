...
host.ready=(async()=>{
 const [theory,effects,kotta,direct,stallionData,mareData,inheritanceValidation]=await Promise.all([
  json('data/theory-master.json'),json('data/nitro-effects.json'),json('data/kotta-pairs.json'),json('data/elaborate-direct-exceptions.json'),json('data/stallions.json'),json('data/default-broodmares.json'),json('data/planner-inheritance-validation.json')
 ]);
 ...
 const master=name=>cloneRecord(masterMap.get(core.key(name)),'switch-master');
 ...
 Object.assign(host,{status:'ready',theory,effects,kotta,directExceptions:direct,stallionData,mareData,knownDifferences,master,stallionStats,mareStats,resolveHorse,record,evaluatePair,calcNitro:engine.calcNitro,danger:engine.danger,theoryFlags:engine.theoryFlags,elaborate:engine.elaborate,deriveChild:engine.deriveChild,deriveChildAncestor:engine.deriveChildAncestor,core:engine,domesticStallions:()=>theory.stallions.map(x=>cloneRecord(x,'switch-master')),broodmares:()=>theory.broodmares.map(x=>cloneRecord(x,'switch-master'))});
 window.DABISTA_BREEDING_ENGINE_STATUS={status:'ok',stallions:theory.stallions?.length||0,broodmares:theory.broodmares?.length||0,directExceptions:direct.pairs?.length||0,kottaPairs:kotta.pairs?.length||0};
 return host;
